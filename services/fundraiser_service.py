...
def list_items(fundraiser_id: str, *, kind: str | None = None) -> list[dict]:
    ...
    # Backfill item_kind for rows created before migration 013 so callers can
    # rely on the field being present in every dict.
    for r in rows:
...
def upsert_item(fundraiser_id: str, item_code: str, *,
    ...
    # Other costs do not trigger the supplier-quote flag — quote threshold is
    # a procurement control for physical goods being sold.
    requires_quote = (
